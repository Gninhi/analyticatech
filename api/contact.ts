 ...
 * ----------------------------------------
 * Architecture: Zero-Trust & Validation Layer.
 */

const POW_DIFFICULTY_PREFIX = '0000'; 

// Simulation d'un store persistant (Dans une vraie prod Vercel, utiliser Vercel KV / Redis)
// Ceci est une protection "best effort" pour l'environnement serverless stateless
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // 2. Rate Limiting (IP Based - Best Effort on Serverless)
  ...
  try {
    ...
    return res.status(200).json({ 
      success: true, 
      message: 'Message reçu et sécurisé.' 
    });
